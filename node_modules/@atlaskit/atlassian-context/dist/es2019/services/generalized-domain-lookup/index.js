...
export function getDomainInContext(subdomain, envType) {
  ...
  const isIsolatedCloudPerimeterType = ISOLATED_CLOUD_PERIMETERS.includes(atlCtxCookieValues.perimeter);
  const isNonIsolatedCloudPerimeterType = NON_ISOLATED_CLOUD_PERIMETERS.includes(atlCtxCookieValues.perimeter);
  if (!isIsolatedCloudPerimeterType && !isNonIsolatedCloudPerimeterType) {
    console.warn(`Atl Ctx cookies are passing in invalid perimeter ${atlCtxCookieValues.perimeter}`);
    return undefined;
  }

  // Form domain for isolated-cloud
  const isIsolatedCloud = isIsolatedCloudPerimeterType && atlCtxCookieValues.icDomain !== undefined;
  if (isIsolatedCloud) {
    const perimeter = atlCtxCookieValues.perimeter;
