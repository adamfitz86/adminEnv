...
export const getValue = (node, context) => {
...
const getValueFromUnaryExpression = (node, context) => {
  if (!isNodeOfType(node, 'UnaryExpression')) {
    return null;
  }
...
export const getValueFromTemplateLiteralRaw = (node, context) => {
  ...
  const combinedString = node.quasis.map((q, i) => {
...
const getValueFromBinaryExpression = (node, context) => {
  ...
  const final = rightValue && leftValue ?
  // eslint-disable-next-line no-eval
  eval(`${leftValue}${operator}${rightValue}`) : null;
  return final;
};
const emRegex = /(.*\d+)em$/;
...
export function getTokenNodeForValue(propertyName, value) {
  ...
  const fallbackValue = propertyName === 'fontFamily' ? {
    value: `${value}`,
    raw: `\`${value}\``
  } : `${value}`;
